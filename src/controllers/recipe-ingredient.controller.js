...
const getRecipeIngredients = async (recipeId) => {
  return recipeIngredientConnector.getRecipeIngredients(recipeId)};
...
// Ingredients that no longer exist are deleted, existing ones are updated and new ones created
const updateRecipeIngredients = async (recipeId, submittedIngredients) => {
  const ingredientsToDelete = (
    await recipeIngredientConnector.getRecipeIngredients(recipeId)).filter(
      (ingredient) => !submittedIngredients.some(
  ...
  const updatedIngredients = submittedIngredients.map((ingredient, index) =>
    Object.assign(ingredient, {ingredient_id: index + 1}));
  ...
  updatedIngredients.forEach((ingredient) => {
    if (ingredient.recipe_ingredient_id) {
      promises.push(recipeIngredientConnector.updateRecipeIngredient(
        ingredient.recipe_ingredient_id, ingredient));
    } else {
