...
describe('HTTP requests to /recipe-ingredients', () => {
    ...
    test('A valid recipe ingredient can be saved and then retrieved and updated', async () => {
        ...
        expect(getResponse.body.name).toEqual('Milk');
        ...
        expect(getResponse2.body.name).toEqual('Soya milk');
    });

    test('Recipe ingredients for a recipe can be retrieved and deleted', async () => {
        const getResponse = await request(app)
